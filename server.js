...
function createMcpServer({ sessionId, config }) {
  ...
  mcpServer.tool(
    "nmapScan",
    {
      target: z
        .string()
        .describe("Domain name, IP address, or CIDR notation to scan (e.g., example.com, 192.168.1.1, 10.0.0.0/24)"),
      flags: z
        .string()
        .optional()
        .default("-T4 -p 1-1000")
        .describe(
          "Nmap scanning flags. Common options: -T4 (timing), -p 1-1000 (port range), -sS (SYN scan), -A (aggressive scan)",
        ),
      outputFormat: z
        .enum(["xml", "normal", "grepable"])
        .optional()
        .default("xml")
        .describe("Output format: xml (structured), normal (human-readable), or grepable (grep-friendly)"),
    },
  ...
  mcpServer.tool("getInfo", z.object({}), async () => {
    return {
      content: [
        {
          type: "text",
...
app.get("/", (req, res) => {
  res.json({
    message: "Nmap MCP Server is running",
    version: "1.0.0",
    endpoints: {
      health: "/health",
      mcp: "/mcp",
